...
precontent:function (Diuse){
    ...
    if(Diuse.enable){
        ...
        download_filter=function(){
            lib.init.js(url,'files',function(){
                ...
                var download1=function(){
                    var url_search=file_url+tianshu_list[0];
                ...
                var download2=function(){
                    var url_search=file_url+mp3_list[0];
                    game.readFile(url_search,function(){
                        num++;
                ...
                var download3=function(){
                    var url_search=file_url+css_list[0];
                    game.readFile(url_search,function(){
                        num++;
                ...
                var download4=function(){
                    var url_search=file_url+static_list[0];
                    game.readFile(url_search,function(){
                        num++;
        ...
        download_beta_version=function(){
            var online_version;
            var httpRequest = new XMLHttpRequest();
            httpRequest.open("GET",'https://diuse.coding.net/p/extension/d/noname_extension/git/raw/master/beta_extension/online_version.js',true);
            httpRequest.send(null);
            httpRequest.onreadystatechange=function(){
                if (httpRequest.readyState==4&&httpRequest.status==200){
                    online_version=httpRequest.responseText;
                    ...
                    lib.init.js(url,'version',function(){
                        try {
                            ...
                            if(confirm('本地资源不完整！点击确认重新获取！')){
                                game.download('https://diuse.coding.net/p/extension/d/noname_extension/git/raw/master/beta_extension/files.js','extension/术樱/files.js',function(){},function(){});
                                game.download('https://diuse.coding.net/p/extension/d/noname_extension/git/raw/master/beta_extension/version.js','extension/术樱/version.js',function(){},function(){});
                                game.download('https://diuse.coding.net/p/extension/d/noname_extension/git/raw/master/beta_extension/extension.js','extension/术樱/extension.js',function(){
                                    game.saveConfig('Diuse_local_version',online_version);
                                    Diuse_Button=true;
                                    alert('下载完成，重启生效');
                                },function(){
        ...
        skinList=function(){
            lib.init.js(url,'files',function(){
                ...
                var down=function(){
                    ...
                    url_search=file_url+skin_list[0]+'/'+skin_num[0]+'.jpg';
            ...
            window.resolveLocalFileSystemURL(lib.assetURL,function(entry){
                entry.getFile(filename,{},function(fileEntry){
                    fileEntry.file(function(){
                        var fso = require('fs');
                        fso.copyFile(lib.assetURL+'/extension/术樱/a.txt',lib.assetURL+'extension/术樱/1/b.txt',(err)=>{
                            if (err) { 
        ...
        lib.element.player.isMaxBasicCards=function(equal){ //基本牌最多
            for(var i=0;i<game.players.length;i++){
                if(game.players[i].isOut()||game.players[i]==this) continue;
                if(equal){
        ...
        lib.element.player.isMinBasicCards=function(equal){ //基本牌最少
            for(var i=0;i<game.players.length;i++){
                if(game.players[i].isOut()||game.players[i]==this) continue;
                if(equal){
                    if(game.players[i].countCards('h',function(card){
        ...
        lib.element.player.isCardColor=function(){ //手牌颜色是否一致
            var cards=this.getCards('h');
            var color=get.color([cards[0],this]);
            if(!cards) return false;
            for(var i=1;i<cards.length;i++){
                if(get.color(cards[i],this)!=color) return false;
            }
            return true;
        };
        ...
        lib.element.player.bossTianshuName=function(num,a){ //全部boss 可以用于调用和匹配boss
            if(a==undefined) a=-99;
        ...
        lib.element.player.removeAllSkills=function(){ //移除全部技能
            for(var i=0;i<game.players.length;i++){
                game.players[i].addSkill('Diuse_removeSkills');
            }
        };

        lib.element.player.addAllSkills=function(){ //恢复全部技能
            for(var i=0;i<game.players.length;i++){
                game.players[i].removeSkill('Diuse_removeSkills');
            }
        };

        lib.element.player.hidePlayer=function(){ //隐藏Boss
            ...
            game.Benghuai3("Benghuai3","崩坏3",{
                ...
                cheat:{ //cfg
                    cfg:function(){
                        ...
                        game.saveConfig('Diuse_local_version','1.7.45.2'); //版本号
                ...
                skill:{
                    skill:{
                        ...
                        Diuse_Diewu:{
                            ...
                            content:function (event,player,targets)
                            {
                                ...
                                //card=game.createCard('sha','red',1);
                                //player.gain(card,'draw');
                                //player.useCard(targets[0],false,card);
                        ...
                        Diuse_Hula:{
                            ...
                            content:function(){
                                ...
                                'step 2'
                                if(player.isMinBasicCards(true)) player.storage.Diuse_Yingtao_Hula_Buff=true;
                            },
                            mod:{
                    ...
                    translate:{
                        ...
                        Diuse_Yvlei:"御雷",
                        Diuse_Yvlei_info:"SP技能，出牌阶段。若你SP大于100则可以消耗20SP开启，并摸X张牌（X为体力上限与手牌数的差且最多为5）然后重置【雷鸣】次数。当你处于开启状态：你造成的雷属性伤害+1；每使用一张伤害类牌消耗10SP；你使用牌无距离限制；若因此消耗100SP则此技能结束。",
                        Diuse_Leidian:"雷电",
            ...
            game.Tianshu=function(英文名,翻译名,obj,扩展包名){
                ...
                lib.skill._Tianshu_checkPoint={ //天书Boss阵亡和奖励
                    ...
                    filter:function(event,player){
                        ...
                        if(bool&&player.bossTianshuName(checkPointNum,_status.Diuse_Tianshu_Difficulty)&&player==event.player&&!event.player.storage.Tianshu_checkPoint) return true;
                        return false;
                    },
                    content:function(){
                        ...
                        if(num==undefined||num==0){ //开始时召唤BOSS
                            ...
                            for(var i=0;i<game.players.length;i++){
                                if(game.players[i]==game.boss){　
                                    var name=player.bossTianshuName(-1);
                            ...
                            if(bool){ //进入
                                var name1=player.bossTianshuName(-2,num2);
                                _status.Diuse_Tianshu_Bosslist.push(name1);
                                game.addBossFellow(newSeat,name1); //召唤4号位第二关Boss num2是难度
                                var name2=player.bossTianshuName(-2,num2);
                                _status.Diuse_Tianshu_Bosslist.push(name2);
                                game.changeBoss(name2); //6号位Boss
                                if(lib.config.extension_术樱_tianshu_addBoss&&_status.Diuse_Xvni_Bool==false){
                                    var name3=player.bossTianshuName(-2,num2);
                                ...
                                if(num2==1){game.bossGainMaxHpCard(0,1);} else if(num2==2){game.bossGainMaxHpCard(0,2);} else {game.bossGainMaxHpCard(0,3);}
                                if(_status.gameMe){
                            ...
                            var nextCheckPoint=player.bossTianshuName(num,num2); //阵亡判断 同第一关
                            if(nextCheckPoint){
                                if(trigger.source!=undefined) trigger.source.hp!=trigger.source.maxHp?trigger.source.recover():trigger.source.draw(2);
                            ...
                            if(bool){
                                var name1=player.bossTianshuName(-3,num2);
                                _status.Diuse_Tianshu_Bosslist.push(name1);
                                game.addBossFellow(newSeat,name1);
                                var name2=player.bossTianshuName(-3,num2);
                                _status.Diuse_Tianshu_Bosslist.push(name2);
                                game.changeBoss(name2);
                                if(lib.config.extension_术樱_tianshu_addBoss&&_status.Diuse_Xvni_Bool==false){
                                    var name3=player.bossTianshuName(-3,num2);
                                    _status.Diuse_Tianshu_Bosslist.push(name3);
                                    game.addBossFellow(newSeat+1,name3);
                                }
                                game.tianshuNewBoss(_status.currentPhase.next,num,num2);
                                if(num2==1){game.bossGainMaxHpCard(0,2);} else if(num2==2){game.bossGainMaxHpCard(0,3);} else {game.bossGainMaxHpCard(0,4);}
                                if(_status.gameMe){
                                    game.boss._trueMe=game.me;
                                    _status.gameMe=false;
                                }
                            ...
                            var nextCheckPoint=player.bossTianshuName(num,num2);
                            if(nextCheckPoint){
                                if(trigger.source!=undefined) trigger.source.hp!=trigger.source.maxHp?trigger.source.recover():trigger.source.draw(2);
                                var gameBoss=player.gameBossAllDie();
                                if(gameBoss){
                            ...
                            if(bool){
                                var name1=player.bossTianshuName(-4,num2);
                                _status.Diuse_Tianshu_Bosslist.push(name1);
                                game.addBossFellow(newSeat,name1);
                                var name2=player.bossTianshuName(-4,num2);
                                _status.Diuse_Tianshu_Bosslist.push(name2);
                                game.changeBoss(name2);
                                if(lib.config.extension_术樱_tianshu_addBoss&&_status.Diuse_Xvni_Bool==false){
                                    var name3=player.bossTianshuName(-4,num2);
                                    _status.Diuse_Tianshu_Bosslist.push(name3);
                                    game.addBossFellow(newSeat+1,name3);
                                }
                                game.tianshuNewBoss(_status.currentPhase.next,num,num2);
                                if(num2==1){game.bossGainMaxHpCard(0,3);} else if(num2==2){game.bossGainMaxHpCard(0,4);} else {game.bossGainMaxHpCard(0,5);}
                                if(_status.gameMe){
                                    game.boss._trueMe=game.me;
                                    _status.gameMe=false;
                                }
                            ...
                            if(bool){
                                var name1=player.bossTianshuName(-5,num2);
                                _status.Diuse_Tianshu_Bosslist.push(name1);
                                game.addBossFellow(newSeat,name1);
                                var name2=player.bossTianshuName(-5,num2);
                                _status.Diuse_Tianshu_Bosslist.push(name2);
                                game.changeBoss(name2);
                                if(lib.config.extension_术樱_tianshu_addBoss&&_status.Diuse_Xvni_Bool==false){
                                    var name3=player.bossTianshuName(-5,num2);
                                    _status.Diuse_Tianshu_Bosslist.push(name3);
                                    game.addBossFellow(newSeat+1,name3);
                                }
                                game.tianshuNewBoss(_status.currentPhase.next,num,num2);
                            ...
                            var nextCheckPoint=player.bossTianshuName(5,num2);
                            if(nextCheckPoint){
                                if(trigger.source!=undefined) trigger.source.hp!=trigger.source.maxHp?trigger.source.recover():trigger.source.draw(2);
                                var test=1,gameBoss;
                                if(test==1){
                        ...
                        if(_status.Diuse_Tianshu_checkPoint<=10) game.playerDead(); //先让玩家阵亡角色复活
                        ...
                        for(var D=0;D<game.players.length;D++){
                            ...
                            if(game.players[D].bossTianshuName(0)) continue;
                        ...
                        if(_status.Diuse_Tianshu_checkPoint>5) if(parseInt(_status.Diuse_Tianshu_checkPoint%5)!=0){
                            player.hidePlayer(); event.goto(0);
                        } 
                        'step 4'
                        var off;
                    ...
                    subSkill:{
                        ...
                        Die:{ //玩家阵亡
                            ...
                            filter:function(event,player){
                                var checkPointNum=_status.Diuse_Tianshu_checkPoint,bool=_status.Diuse_Tianshu_Bool;
                                if(checkPointNum>=6) checkPointNum=5;
                                if(bool&&!player.bossTianshuName(checkPointNum,_status.Diuse_Tianshu_Difficulty)&&player==event.player&&!player.storage.Tianshu_checkPoint) return true;
                                return false;
                            },
                            content:function(){
                                var num=_status.Diuse_Tianshu_checkPoint;
                                ...
                                for(var i=0;i<game.players.length;i++){
                                    if(game.players[i]==player) continue;
                                    if(!game.players[i].bossTianshuName(num,num2)&&!game.players[i].bossTianshuName(0,num2)){
                                        gamePlayerNum++;
                                    }
                                }
                                if(gamePlayerNum==0){
            ...
            game.Tianshu("Tianshu","天书乱斗",{
                character:{
                    character:{
                        ...
                        Boss_Diuse_Huanjin:["male","",0,["Boss_Diuse_Huanjin_Go"/*,"Boss_Diuse_Huanjin_intro1","Boss_Diuse_Huanjin_intro2","Boss_Diuse_Huanjin_intro3","Boss_Diuse_Huanjin_intro4","Boss_Diuse_Huanjin_intro5"*/],["boss"],"qun"],
                ...
                game:{ //Boss自定义函数处
                    ...
                    bossGainMaxHpCard:function(maxHp,card){
                        for(var i=0;i<game.players.length;i++){
                            if(game.players[i].isFriendOf(game.boss)){
                                game.players[i].addSkill('Diuse_removeSkills');
                                if(maxHp){
                    ...
                    tianshuNewBoss:function(nextName,checkPoint,livePlayer){ //重置回合等  天书调用
                        ...
                        if(nextName.bossTianshuName(0,0)) nextName=game.boss;
                        if(nextName.bossTianshuName(checkPoint,livePlayer)) nextName=game.boss;
                        game.resetSkills();
                        _status.paused=false;
                ...
                boss:{
                    ...
                    Boss_Diuse_Huanjin:{
                        ...
                        init:function(){
                            ...
                            lib.inpile.sort(lib.sort.card);
                            _status.Diuse_newCardList=[
                                ['sha','black',1,'fire'],['sha','red',3,'thunder'],['sha','black',5,'ice'],['sha','red',9,''],['sha','red',10,'thunder'],['sha','red',11,''],['sha','red',5,''],
                                ['shan','black',2,''],['shan','black',4,''],['shan','black',6,''],['shan','black',7,''],['shan','black',13,''],
                                ['tao','black',13,''],['tao','black',1,''],['tao','black',5,''],
                                ['jiu','red',13,''],
                                ['shunshou','black',4,''],
                                ['guohe','black',7,''],
                                ['tiesuo','red',13,''],
                            ...
                            _status.Diuse_newCard=[..._status.Diuse_newCardList];
                        },
                        loopFirst:function(){
                            return game.boss;
                        },
                        ...
                        livePlayer:{
                            ...
                            content:function(){
                                ...
                                if(game.me==game.boss){
                                    ...
                                    game.boss.nextSeat.nextSeat.changeSeat(3);
                                    if(game.boss.previousSeat.bossTianshuName(0)){ 
                                ...
                                "step 3"
                                for(var i=0;i<game.players.length;i++){
                                    if(game.players[i].bossTianshuName(0)){
                            ...
                            content:function(){
                                for(var i=game.players.length-1;i>=0;i--){
                                    ...
                                    if(i!=0){
                                        ...
                                        _status.Diuse_PVEPlayer.addSkill('Diuse_Save');
                                        _status.Diuse_PVEPlayer.addSkill('Diuse_Load');
                        ...
                        Diuse_newCard:{
                            ...
                            content:function(){
                                ...
                                for(var i=0;i<num;i++){
                                    ...
                                    var rand=game.randomNum(_status.Diuse_newCard.length-1,0);
                            ...
                            subSkill:{
                                ...
                                PlayerDamage:{ //阴天：涛神或曹娥受到伤害后，其进行判定，若结果为红桃，回复1点体力。
                                    ...
                                    filter:function(event,player){
                                        ...
                                        if(player.storage.Boss_Tianqi==undefined) return false;
                                        var stor=player.storage.Boss_Tianqi[0];
                                        var name=event.player.bossTianshuName(255,0);
            ...
            game.Diy("Diuse_Diy","DIY",{
                ...
                skill:{
                    skill:{
                        ...
                        Diuse_Lianzhu:{
                            ...
                            content:function(){
                                ...
                                })==target.countCards('h')) cardNum++;
                                if(player.countCards('h')==0) cardNum++;
                                if(player.isCardColor()) cardNum++;
                                if(target.isCardColor()) cardNum++;
                                'step 0'
                                ...
                                if(get.color(cards[0],player)=='red'){
                                    ...
                                    target.chooseToDiscard('he',cardNum,'弃置'+cardNum+'张牌，或令'+get.translation(player)+'摸'+cardNum+'张牌').set('goon',get.attitude(target,player)<0).set('ai',function(card){
                        ...
                        Diuse_Xiahui:{
                            ...
                            filter:function(event,player){
                                if(event.type!='discard') return false;
                                for(var i=0;i<event.cards2.length;i++){
                                    if(get.position(event.cards2[i])=='d'&&event.player.getHistory('lose',function(evt){
                                        for(var i in evt.gaintag_map){
                                            if(evt.gaintag_map[i].contains('Diuse_Xiahui')) return true;
                                        }
                    ...
                    translate:{
                        ...
                        Diuse_Xiahui_info:"锁定技，你的黑色手牌不计入手牌上限；当有其他角色获得你的黑色牌后，其于下次扣减体力前不能使用，打出，弃置这些牌；当有角色因弃置而失去你的黑色牌时，其失去一点体力；当有角色获得你的一张红色牌后你可以令其使用之",
                    },
                },
            },"术樱");
        }
